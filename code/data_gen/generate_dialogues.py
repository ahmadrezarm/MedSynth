...
MODEL_PATH= "/model-weights/Llama-3.3-70B-Instruct"

def main():
    counter= 1
    #openai_client= dial_generation_functions.initialize_openai_client()
    model, tokenizer = dial_generation_functions.load_local_model(MODEL_PATH)
    for filename in os.listdir(path_to_input):
        ...
        if filename.endswith(".csv"):
            if any(desc in filename for desc in top_50_icd10_desc):
                ...
                note_df_with_dial= dial_generation_functions.generate_dialouge_for_df_qwen(note_df= note_df,
                                                                                    model = model,
                                                                                    tokenizer= tokenizer)
