...
""" 
# Directly specify the components to enable
medspacy_pipes = ['medspacy_quickumls', 'medspacy_context', 'medspacy_sectionizer', 'medspacy_target_matcher']
# Load the medspacy model with the specified components
nlp = medspacy.load(enable=medspacy_pipes, quickumls_path='../medspacy_test/')
...
"""
def apply_chatgpt(messages, temperature=0.5, max_tokens=-1, presence_penalty=0, frequency_penalty=0, method="gpt-4-1106-preview"): # gpt-4
  cnt = 0
  while cnt < 5:
    cnt += 1
    if max_tokens != -1:
...
def apply_chatgpt(messages, temperature=0.5, max_tokens=-1, presence_penalty=0, frequency_penalty=0, method="gpt-4"): #  gpt-4-1106-preview
  ...
  while cnt < 5:
    ...
    if max_tokens != -1:
      try:
        completion = client.chat.completions.create(model=method,
        messages=messages,
        max_tokens=max_tokens, 
        temperature=temperature,
        presence_penalty=presence_penalty,
...
def main():
  ...
  file = open(f'/Users/ahmadrezaie/DalPhD/Research/note_taking_v2/data/output/NoteChat_gpt4/{args.index}.txt', 'w')
  file.write(conv)
  file.close()
