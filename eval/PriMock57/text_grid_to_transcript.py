...
import primock_utils #from .  


def get_combined_transcript(transcript_path_doctor, transcript_path_patient):
    utterances_doctor = primock_utils.get_utterances_textgrid(transcript_path_doctor)
    utterances_patient = primock_utils.get_utterances_textgrid(transcript_path_patient)
    ...
    return [f"{u['speaker']}: {primock_utils.strip_transcript_tags(u['text'])}"
